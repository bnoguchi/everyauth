...
openidModule.submodule('googlehybrid')
  ...
  .verifyAttributes(function(req,res) {
    var p = this.Promise()
        oa = this.oa;
    this.relyingParty.verifyAssertion(req, function (error,userAttributes) {
  ...
  .sendToAuthenticationUri(function(req,res) {
    ...
    this.relyingParty.authenticate('http://www.google.com/accounts/o8/id', false, function(error,authenticationUrl){
     if(error) return p.fail([error]);
