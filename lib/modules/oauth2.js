var everyModule = require('./everymodule')
  ...
  , getSession = require('../sharedSteps').getSession
...
everyModule.submodule('oauth2')
  ...
  .configurable({
    ...
    , postAccessTokenParamsVia: '"qs" to POST the params to the access ' + 
                'token endpoint as a querysting; "form" to POST the params to ' +
  ...
  .getAccessToken( function getAccessToken (code) {
    ...
    opts.url = url;
    request[this._accessTokenHttpMethod](opts, function (err, res, data) {
      if (err) return p.fail(err, data);
      // TODO Need better coverage over statusCodes
      if (res.statusCode === 400) return p.fail(data);
