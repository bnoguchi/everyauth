...
var materializedMethods = {
     ...
     * @returns {Promise}
     */
  , _bind: function (priorPromise, nextStep, nextMiddleware) {
      var nextPromise = this.module.Promise()
        , seq = this;

      priorPromise.callback( function () {
        var resultPromise = nextStep.exec(seq, nextMiddleware);
  ...
  , start: function () {
      ...
      var nextMiddleware = this._transposeArgs(arguments);

      // Pipe through the steps
      var priorStepPromise = steps[0].exec(this, nextMiddleware);

      // If we have a breakTo
      if (!priorStepPromise) return;

      for (var i = 1, l = steps.length; i < l; i++) {
        priorStepPromise = this._bind(priorStepPromise, steps[i], nextMiddleware);
      }
  ...
  , _transposeArgs: function (args) {
      ...
      return (args.length > firstStep.accepts.length) ? args[args.length-1] : function() {};
    }
};
