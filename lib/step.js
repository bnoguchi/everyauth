...
Step.prototype = {
     ...
     * @returns {Promise}
     */
    exec: function (seq, nextMiddleware) {
      ...
      try {
        ...
        if (breakTo.isSeq) {
          ...
          breakTo.initialArgs.push(nextMiddleware);
          ...
          // TODO Scope this fn
          if(_module._connectErrorHandling) return nextMiddleware(breakTo);
      ...
      if (promises && promises.length &&
        ...
        // TODO Scope this fn
        var error = new Error('Step ' + this.name + ' of `' + _module.name +
        ...
        if (_module._connectErrorHandling) return nextMiddleware(error);
        errorCallback(error);
      }
      ...
      if (_module._connectErrorHandling) {
        ret.nextMiddleware = nextMiddleware;
      }
      else {
